...
class SO_MetaKeywords extends SO_Module {
	
	function get_module_title() { return __('Meta Keywords Editor', 'seo-optimization'); }
	function get_menu_title()   { return __('Meta Keywords', 'seo-optimization'); }
	...
	function get_admin_page_tabs() {
		return array_merge(
			...
			, $this->get_meta_edit_tabs(array(
				...
				, 'label' => __('Meta Keywords', 'seo-optimization')
			))
		);
	}
	...
	function defaults_tab() {
		...
		$posttypenames = sowp::get_post_type_names();
		foreach ($posttypenames as $posttypename) {
			...
			if (post_type_supports($posttypename, 'editor'))
				$checkboxes["auto_keywords_posttype_{$posttypename}_words"] = __('The %d most commonly-used words', 'seo-optimization');
		...
		$this->textarea('global_keywords', __('Sitewide Keywords', 'seo-optimization') . '<br /><small><em>' . __('(Separate with commas)', 'seo-optimization') . '</em></small>');
	...
	function home_tab() {
		$this->admin_form_table_start();
		$this->textarea('home_keywords', __('Blog Homepage Meta Keywords', 'seo-optimization'), 3);
		$this->admin_form_table_end();
	}
	...
	function head_tag_output() {
			...
			if ($posttypename = get_post_type()) {
				...
				foreach ($taxnames as $taxname) {
					if ($this->get_setting("auto_keywords_posttype_{$posttypename}_tax_{$taxname}", false)) {
						...
						$terms = soarr::flatten_values($terms, 'name');
						$terms = implode(',', $terms);
						$kw .= ',' . $terms;
					}
				}
				...
				if ($this->get_setting("auto_keywords_posttype_{$posttypename}_words", false)) {
					...
					$stopwords = soarr::explode_lines($this->get_setting('words_to_remove', array(), 'slugs'));
		...
		$kw = soarr::array_unique_i($kw); //Remove duplicate keywords
		$kw = implode(',', $kw);
	...
	function postmeta_fields($fields) {	
		$fields['25|keywords'] = $this->get_postmeta_textbox('keywords', __('Meta Keywords:<br /><em>(separate with commas)</em>', 'seo-optimization'));
		return $fields;
	}
	
	function postmeta_help($help) {
		$help[] = __('<strong>Keywords</strong> &mdash; The value of the meta keywords tag. The keywords list gives search engines a hint as to what this post/page is about. Be sure to separate keywords with commas, like so: <samp>one,two,three</samp>.', 'seo-optimization');
		return $help;
	}
...
", 'seo-optimization')));
		...
		$screen->add_help_tab(array(
			  'id' => 'so-meta-keywords-troubleshooting'
