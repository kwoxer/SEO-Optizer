...
class SO_Widgets extends SO_Module {
	
	function get_module_title() { return __('SEO Optimization Widgets', 'seo-optimization'); }
	...
	function register_widgets() {
		register_widget('SO_Widget_SiloedTerms');
		
		if ($this->plugin->module_exists('footer-autolinks'))
			register_widget('SO_Widget_FooterLinks');
	}
}
...
class SO_Widget_SiloedTerms extends WP_Widget {
	
	function __construct() {
		$widget_ops = array( 'description' => __( "On category archives, displays a list of child categories and/or posts in the category. Displays a list of top-level categories everywhere else. Powered by the SEO Optimization plugin.", 'seo-optimization' ) );
		parent::__construct('so_siloed_terms', __('Siloed Categories', 'seo-optimization'), $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		if (sowp::is_tax($current_taxonomy)) {
...
?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e( 'Title:', 'seo-optimization' ); ?></label>
		...
		<label for="<?php echo $this->get_field_id('count'); ?>"><?php _e( 'Show post counts', 'seo-optimization' ); ?></label>		
		...
		<label for="<?php echo $this->get_field_id('use_desc_for_title'); ?>"><?php _e( 'Use term descriptions in title attributes', 'seo-optimization' ); ?></label></p>
		
		<p><label for="<?php echo $this->get_field_id('taxonomy'); ?>"><?php _e('Taxonomy:', 'seo-optimization') ?></label>
...
class SO_Widget_FooterLinks extends WP_Widget {
	
	function __construct() {
		$widget_ops = array( 'description' => __( "Add this widget to display Deeplink Juggernaut&#8217;s Footer Links in a widget area of your choosing rather than the default wp_footer section. Powered by the SEO Optimization plugin.", 'seo-optimization' ) );
		parent::__construct('so_footer_autolinks', __('Footer Links', 'seo-optimization'), $widget_ops);
	}
	...
	function widget( $args, $instance ) {
		...
		if ($seo_optimization->call_module_func('footer-autolinks', 'autolink_footer', $unused, $args))
			$seo_optimization->set_module_var('footer-autolinks', 'already_outputted', true);
	...
	function form( $instance ) {
		...
		$display = empty($instance['display']) ? 'list' : $instance['display'];
?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e( 'Title: <em>(optional)</em>', 'seo-optimization' ); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></p>
		
		<p>
			<label><input type="radio" name="<?php echo $this->get_field_name('display'); ?>" value="list" <?php checked('list', $display); ?>/> <?php _e('Display as a list', 'seo-optimization'); ?></label><br />
			<label><input type="radio" name="<?php echo $this->get_field_name('display'); ?>" value="usersettings" <?php checked('usersettings', $display); ?>/> <?php
				$seo_optimization->call_module_func('footer-autolinks-settings', 'get_admin_url', $formats_url);
				printf(__('Use my <a href="%s" target="_blank">footer link HTML formats</a>', 'seo-optimization'), $formats_url);
			?></label>
		</p>
<?php
	}
