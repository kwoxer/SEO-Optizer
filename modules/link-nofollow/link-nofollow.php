...
class SO_LinkNofollow extends SO_Module {
	
	function get_module_title() { return __('Nofollow Manager', 'seo-optimization'); }
	function get_default_status() { return SO_MODULE_DISABLED; }
	...
	function admin_page_contents() {
		...
		$this->admin_form_subheader(__('Add the nofollow attribute to...', 'seo-optimization'));
		$this->checkboxes(array(
				  'nofollow_adjacent_post' => __("Adjacent post links", 'seo-optimization')
				, 'nofollow_category_loop' => __("Category links (after posts)", 'seo-optimization')
				, 'nofollow_category_list' => __("Category links (in lists)", 'seo-optimization')
				, 'nofollow_comment_popup' => __("Comment anchor links", 'seo-optimization')
				, 'nofollow_comment_feed' => __("Comment feed links", 'seo-optimization')
				, 'nofollow_date_archive' => __("Date-based archive links", 'seo-optimization')
				, 'nofollow_paged' => __("Pagination navigation links (all)", 'seo-optimization')
				, 'nofollow_paged_home' => __("Pagination navigation links (on blog home only)", 'seo-optimization')
				, 'nofollow_post_more' => __("&#8220;Read more&#8221; links", 'seo-optimization')
				, 'nofollow_register' => __("Registration link", 'seo-optimization')
				, 'nofollow_login' => __("Login link", 'seo-optimization')
				, 'nofollow_tag_loop' => __("Tag links (after posts)", 'seo-optimization')
				, 'nofollow_tag_list' => __("Tag links (in lists and clouds)", 'seo-optimization')
			));
	...
	function postmeta_fields($fields, $screen) {
		...
		if (strcmp($screen, 'page') == 0)
			$fields['40|nofollow'] = $this->get_postmeta_checkbox('nofollow', __('When displaying page lists, nofollow links to this page', 'seo-optimization'), __('Nofollow:', 'seo-optimization'));
