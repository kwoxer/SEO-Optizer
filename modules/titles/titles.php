...
class SO_Titles extends SO_Module {
	
	function get_module_title() { return __('Title Rewriter', 'seo-optimization'); }
	...
	function init() {
		...
		add_filter('so_postmeta_help', array(&$this, 'postmeta_help'), 10);
	}
	...
	function get_admin_page_tabs() {
		return array_merge(
			  array(
				...
				, array('title' => __('Settings', 'seo-optimization'), 'id' => 'so-settings', 'callback' => 'settings_tab')
				)
			, $this->get_meta_edit_tabs(array(
				  'type' => 'textbox'
				, 'name' => 'title'
				, 'term_settings_key' => 'taxonomy_titles'
				, 'label' => __('Title Tag', 'seo-optimization')
			))
		);
	}
	...
	function settings_tab() {
		...
		$this->checkbox('terms_ucwords', __('Convert lowercase category/tag names to title case when used in title tags.', 'seo-optimization'), __('Title Tag Variables', 'seo-optimization'));
		$this->radiobuttons('rewrite_method', array(
			  'ob' => __('Use output buffering &mdash; no configuration required, but slower (default)', 'seo-optimization')
			, 'filter' => __('Use filtering &mdash; faster, but configuration required (see the &#8220;Settings Help&#8221; dropdown for details)', 'seo-optimization')
		), __('Rewrite Method', 'seo-optimization'));
	...
	function get_default_settings() {
		...
		return array(
			  'title_home' => __('{blog}', 'seo-optimization')
			, 'title_single' => __('{post} | {blog}', 'seo-optimization')
			, 'title_page' => __('{page} | {blog}', 'seo-optimization')
			, 'title_category' => __('{category} | {blog}', 'seo-optimization')
			, 'title_tag' => __('{tag} | {blog}', 'seo-optimization')
			, 'title_day' => __('Archives for {month} {day}, {year} | {blog}', 'seo-optimization')
			, 'title_month' => __('Archives for {month} {year} | {blog}', 'seo-optimization')
			, 'title_year' => __('Archives for {year} | {blog}', 'seo-optimization')
			, 'title_author' => __('Posts by {author} | {blog}', 'seo-optimization')
			, 'title_search' => __('Search Results for {query} | {blog}', 'seo-optimization')
			, 'title_404' => __('404 Not Found | {blog}', 'seo-optimization')
			, 'title_paged' => __('{title} - Page {num}', 'seo-optimization')
	...
	function get_supported_settings() {
		return array(
			  'title_home' => __('Blog Homepage Title', 'seo-optimization')
			, 'title_single' => __('Post Title Format', 'seo-optimization')
			, 'title_page' => __('Page Title Format', 'seo-optimization')
			, 'title_category' => __('Category Title Format', 'seo-optimization')
			, 'title_tag' => __('Tag Title Format', 'seo-optimization')
			, 'title_day' => __('Day Archive Title Format', 'seo-optimization')
			, 'title_month' => __('Month Archive Title Format', 'seo-optimization')
			, 'title_year' => __('Year Archive Title Format', 'seo-optimization')
			, 'title_author' => __('Author Archive Title Format', 'seo-optimization')
			, 'title_search' => __('Search Title Format', 'seo-optimization')
			, 'title_404' => __('404 Title Format', 'seo-optimization')
			, 'title_paged' => __('Pagination Title Format', 'seo-optimization')
		);
	}
	...
	function after_header() {
		if ($this->should_rewrite_title()) {
			...
			if (count($handlers) > 0 && strcasecmp($handlers[count($handlers)-1], 'SO_Titles::change_title_tag') == 0)
				ob_end_flush();
			else
				so_debug_log(__FILE__, __CLASS__, __FUNCTION__, __LINE__, "Other ob_list_handlers found:\n".print_r($handlers, true));
		}
	}
	...
	function get_title() {
		...
		if (is_category()) {
			...
			$cat_titles = so_lang_implode($categories, 'name');
		...
		if (strlen($cat_title) && $this->get_setting('terms_ucwords', true))
			$cat_title = sostr::tclcwords($cat_title);
		...
		if (is_tag()) {
			...
			if ($this->get_setting('terms_ucwords', true))
				$tag_title = sostr::tclcwords($tag_title);
		}
		...
		$variables = array(
			...
			, '{tags}' => so_lang_implode(get_the_tags($post_id), 'name', true)
			, '{daynum}' => $daynum
			, '{day}' => $day
			, '{monthnum}' => $monthnum
			, '{month}' => $month
			...
			, '{query}' => so_esc_attr(get_search_query())
			, '{ucquery}' => so_esc_attr(ucwords(get_search_query()))
	...
	function postmeta_fields($fields) {
		$id = "_so_title";
		$value = so_esc_attr($this->get_postmeta('title'));
		$fields['10|title'] = "<tr class='textarea' valign='top'>\n<th scope='row'><label for='$id'>".__('Page Title:', 'seo-optimization')."</label></th>\n"
			. "<td><textarea name='$id' id='$id' class='regular-text' cols='60' rows='1' tabindex='2'"
			. "onkeyup=\"javascript:document.getElementById('so_meta_title_charcount').innerHTML = document.getElementById('_so_title').value.length\">$value</textarea>"."<br />".
			sprintf(__('You&#8217;ve entered %s characters. Title Tag should be 60 signs long. Notice that the subtitle has additional influence on this value!', 'seo-optimization'), "<strong id='so_meta_title_charcount' >".strlen($value)."</strong>");
		return $fields;
	}
	
	function postmeta_help($help) {
		$help[] = __('<strong>Title Tag</strong> &mdash; The exact contents of the &lt;title&gt; tag. The title appears in visitors&#8217; title bars and in search engine result titles. If this box is left blank, then the <a href="admin.php?page=so-titles" target="_blank">default post/page titles</a> are used.', 'seo-optimization');
		return $help;
	}
...
", 'seo-optimization')));
	...
	$screen->add_help_tab(array(
			  'id' => 'so-titles-vars'
			, 'title' => __('Formats &amp; Variables', 'seo-optimization')
			, 'content' => __("
