...
class SO_InternalLinkAliases extends SO_Module {
	...
	function init() {
		add_filter('so_custom_update_postmeta-aliases', array(&$this, 'save_post_aliases'), 10, 4);
		...
		add_action('so_do_robotstxt', array(&$this, 'block_aliases_dir'));
		add_filter('so_get_setting-internal-link-aliases-alias_dir', array(&$this, 'filter_alias_dir'));
	}
	...
	function get_module_title() { return __('Link Mask Generator', 'seo-optimization'); }
	
	function get_settings_key() { return 'internal-link-aliases'; }
	
	function get_admin_page_tabs() {
		return array(
			  array('id' => 'aliases',  'title' => __('Aliases', 'seo-optimization'),  'callback' => 'editor_tab')
			, array('id' => 'settings', 'title' => __('Settings', 'seo-optimization'), 'callback' => 'settings_tab')
		);
	}
	...
	function editor_tab() {
		...
		if ($num_aliases > 0) {
			$this->admin_subheader(__('Edit Existing Aliases', 'seo-optimization'));
			$this->aliases_form(0, $aliases);
		}
		
		$this->admin_subheader(__('Add a New Alias', 'seo-optimization'));
	...
	function aliases_form($start_id = 0, $aliases, $delete_option = true) {
		...
		//Set headers
		$headers = array(
			  'alias-from' => __('Actual URL', 'seo-optimization')
			, 'alias-to' => __('Alias URL', 'seo-optimization')
			, 'alias-posts' => __('Only on This Post&hellip; (optional)', 'seo-optimization')
		);
		if ($delete_option) $headers['alias-delete'] = __('Delete', 'seo-optimization');
		...
		foreach ($aliases as $id => $alias) {
			...
			$cells = array(
				...
				, 'alias-to' => "
<table><tr>
	<td class='so-alias-to-dir'>/$alias_dir/</td>
	<td class='so-alias-to-slug'>" . $this->get_input_element('textbox', "alias_{$i}_to", $alias['to']) . "</td>
	<td class='so-alias-to-test'>[<a href='$alias_url' target='_blank'>" . __('Test', 'seo-optimization') . "</a>]</td>
</tr></table>"
	...
	function settings_tab() {
		...
		$this->textbox('alias_dir', __('Alias Directory', 'seo-optimization'), $this->get_default_setting('alias_dir'));
		if ($this->plugin->module_exists('link-nofollow'))
			$this->checkbox('nofollow_aliased_links', __('Nofollow aliased links', 'seo-optimization'), __('Link Attributes', 'seo-optimization'));
		$this->admin_form_table_end();
	}
	
	function filter_alias_dir($alias_dir) {
		return trim(sostr::preg_filter('a-zA-Z0-9_/', $alias_dir), '/');
	}
	...
	function postmeta_fields($fields) {
		...
		if ($content && preg_match_all('@ href=["\']([^"\']+)["\']@', $content, $matches)) {
			...
			$html = "<tr valign='top'>\n<th scope='row' class='so'>".__('Link Masks:', 'seo-optimization')."</th>\n<td>\n";
			
			$html .= "<table class='widefat'><thead>\n";
			$headers = array(__('URL', 'seo-optimization'), '', __('Mask URL', 'seo-optimization'));
			...
			foreach ($urls as $url) {
				...
				$ht_url = esc_html(sostr::truncate($url, 30));
				...
				$html .= "<tr><td><a href='$a_url' title='$a_url' target='_blank'>$ht_url</a><input type='hidden' name='_so_aliases[$alias_id][from]' value='$a_url' /></td>\n<td>&rArr;</td><td>/$alias_dir/<input type='text' name='_so_aliases[$alias_id][to]' value='$a_alias_to' /></td></tr>\n";
			}
			
			$html .= "</tbody>\n</table>\n";
			
			$html .= '<p><small>' . __('You can stop search engines from following a link by typing in a mask for its URL.', 'seo-optimization') . "</small></p>\n";
	...
	function redirect_aliases() {
		...
		foreach ($aliases as $alias)
			if ($to = $alias['to'])
				if (sourl::equal(sourl::current(), get_bloginfo('url') . "/$alias_dir/$to/"))
					wp_redirect($alias['from']);
	}
	
	function block_aliases_dir() {
		echo '# ';
		_e("Added by SEO Optimization's Link Mask Generator module", 'seo-optimization');
		echo "\n";
		...
		echo '# ';
		_e('End Link Mask Generator output', 'seo-optimization');
		echo "\n\n";
	}
