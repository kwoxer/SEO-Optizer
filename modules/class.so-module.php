...
class SO_Module {
	 ...
	 * @since 1.5
	 * 
	 * @return int Either SO_MODULE_ENABLED, SO_MODULE_SILENCED, or SO_MODULE_HIDDEN.
	 */
	function get_default_status() { return SO_MODULE_ENABLED; }
	...
	/**
	 * Called when SEO Optimization has just been upgraded to a new version.
	 * 
	 * @since 2.1
	 */
	function upgrade() { }
	 ...
	 * @uses get_module_key()
	 * @uses SEO_Optimization::key_to_hook()
	 * 
	...
	function get_admin_url($key = false) {
		...
		$basepage = 'admin.php';
		if ($this->plugin->call_module_func($key, 'get_menu_parent', $custom_basepage) && sostr::endswith($custom_basepage, '.php'))
	...
	function children_admin_pages_form() {
		if (count($this->modules)) {
			...
			$this->print_message('warning', sprintf(__('All the modules on this page have been disabled. You can re-enable them using the <a href="%s">Module Manager</a>.', 'seo-optimization'), $this->get_admin_url('modules')));
	}
	 ...
	 * 
	 * @since 1.5
	 * @uses so_Module::get_child_order()
	 * @uses so_Module::get_module_sobtitle()
	 * 
	 * @param so_Module $a The first module to compare.
	 * @param so_Module $b The second module to compare.
	...
	function get_setting($key, $default=null, $module=null, $sneakpeak=false) {
		...
		$setting = apply_filters("so_get_setting-$module", $setting, $key);
		$setting = apply_filters("so_get_setting-$module-$key", $setting, $key);
	...
	function update_setting($key, $value, $module=null, $array_key=null) {
		...
		$use_custom  = 	apply_filters("so_custom_update_setting-$module-$key", false, $value, $key) ||
						apply_filters("so_custom_update_setting-$module", false, $value, $key);
	 ...
	 */
	function admin_page() {
		if (!apply_filters('so_custom_admin_page-'.$this->get_module_key(), false)) {
	...
	function admin_page_start($icon = 'options-general') {
		...
		if (strcmp($pclass = strtolower(get_parent_class($this)), 'so_module') != 0)
			$class = ' '.str_replace('_', '-', $pclass);
		else
			$class = '';
		
		echo "<div id=\"so-".so_esc_attr($this->get_module_key())."\" class=\"so-module$class\">\n";
		screen_icon($icon);
	...
	function admin_subheader($title, $id=false) {
		if ($id) $id = ' id="' . so_esc_attr($id) . '"';
		echo "<h4 class='so-subheader'$id>$title</h4>\n";
	}
	 ...
	 * @since 0.7
	 * @uses get_admin_url()
	 * @uses SEO_Optimization::plugin_dir_url
	 * 
	...
	function admin_page_tabs($tabs = array(), $table=false) {
		...
		if ($c = count($tabs)) {
			...
			if ($c > 1)
				echo "\n\n<div id='so-tabset' class='so-tabs'>\n";
			...
			foreach ($tabs as $tab) {
				...
				if ($c > 1) {
					//$id = 'so-' . sostr::preg_filter('a-z0-9', strtolower($title));
					echo "<fieldset id='$id'>\n<h3>$title</h3>\n<div class='so-tab-contents'>\n";
				}
			...
			if ($c > 1) {
				...
				echo '<script type="text/javascript" src="'.$this->plugin->plugin_dir_url.'includes/tabs.js?v='.SO_VERSION.'"></script>';
			}
		}
	}
	...
	function admin_footer() {
		printf(__('%1$s | %2$s %3$s by %4$s', 'seo-optimization'),
			$this->get_module_title(),
			'<a href="'.SO_PLUGIN_URI.'" target="_blank">'.__(SO_PLUGIN_NAME, 'seo-optimization').'</a>',
			SO_VERSION,
			'<a href="'.SO_AUTHOR_URI.'" target="_blank">'.__(SO_AUTHOR, 'seo-optimization').'</a>'
		);
	...
	function get_postmeta_edit_tabs($fields) {
		...
		$types = sowp::get_post_type_objects();
		...
		foreach ($types as $type)
			$tabs[] = array(
				...
				, 'id' => 'so-' . $type->name
				, 'callback' => array('meta_edit_tab', 'post', 'so-' . $type->name, $type->name, $type->labels->singular_name, $fields)
			);
		return $tabs;
	}
		...
		foreach ($types as $name => $type) {
			if ($type->labels->name) {
				$tabs[] = array(
					  'title' => $type->labels->name
					, 'id' => 'so-' . $name
					, 'callback' => array('meta_edit_tab', 'term', 'so-' . $name, $name, $type->labels->singular_name, $fields)
				);
			}
		}
		return $tabs;
	...
	function meta_edit_tab($genus, $tab, $type, $type_label, $fields) {
		if (!$this->meta_edit_table($genus, $tab, $type, $type_label, $fields))
			$this->print_message('info', __('Your site currently doesn&#8217;t have any public items of this type.', 'seo-optimization'));
	}
	...
	function meta_edit_table($genus, $tab, $type, $type_label, $fields) {
		...
		if ($is_update = ($this->is_action('update') && !strlen(trim($search)))) {
			foreach ($_POST as $key => $value) {
				...
				if (sostr::startswith($key, $genus.'_'))
					foreach ($fields as $field)
						if (preg_match("/{$genus}_([0-9]+)_{$field['name']}/", $key, $matches)) {
							$id = (int)$matches[1];
							switch ($genus) {
								case 'post': update_post_meta($id, "_so_{$field['name']}", $value); break;
		...
		foreach ($objects as $object) {
			...
			switch ($genus) {
				...
				case 'term':
					...
					$edit_url = sowp::get_edit_term_link($id, $type);
					break;
				default: return false; break;
			}
			
			$view_url = so_esc_attr($view_url);
			$edit_url = so_esc_attr($edit_url);
			
			$actions = array(sprintf('<a href="%s">%s</a>', $view_url, __('View', 'seo-optimization')));
			if ($edit_url)
				$actions[] = sprintf('<a href="%s">%s</a>', $edit_url, __('Edit', 'seo-optimization'));
			...
			foreach ($fields as $field) {
				...
				if ($is_update && $field['type'] == 'checkbox' && $value == '1' && !isset($_POST[$inputid]))
					switch ($genus) {
						case 'post': delete_post_meta($id, "_so_{$field['name']}"); $value = 0; break;
	...
	function get_input_element($type, $name, $value=null, $extra=false, $inputid=true) {
		...
		if (strlen($inputid)) $inputid = " id='".so_esc_attr($inputid)."'";
		
		//Get HTML element
		switch ($type) {
			case 'textbox':
				$value = so_esc_editable_html($value);
				$placeholder = $extra ? " placeholder='" . so_esc_attr($extra) . "'" : '';
			...
			case 'checkbox':
				...
				if (is_string($extra)) {
					$extra = so_esc_html($extra);
					$html = "<label>$html&nbsp;$extra</label>";
				}
				return $html;
				break;
			case 'dropdown':
				$html = "<select name='$name'$inputid onchange='javascript:so_toggle_select_children(this)' class='dropdown'>";
				if (is_array($extra)) $html .= sohtml::option_tags($extra, $value); else $html .= $extra;
				$html .= "</select>";
				return $html;
				break;
			case 'hidden':
	...
	function get_admin_form_subsection($field, $current_value, $trigger_value, $html) {
		...
		$field = so_esc_attr($field);
		$trigger_value = so_esc_attr($trigger_value);
		$html = "<div class='so_{$field}_{$trigger_value}_subsection$hidden'>$html</div>";
		return $html;
	}
	...
	function admin_form_start($header = false, $table = true, $form = true) {
		...
		if ($form) {
			...
			if ($this->is_action('update')) $this->print_message('success', __('Settings updated.', 'seo-optimization'));
			echo "<form id='so-admin-form' method='post' action='?page=$hook'>\n";
...
<?php
	...
	function table_column_headers($headers) {
		...
		foreach ($headers as $class => $header) {
			$class = is_numeric($class) ? '' : " class='so-$mk-$class so-$class'";
	...
	function table_cells($cells) {
		...
		if (count($this->get_admin_table_columns())) {
			...
			foreach ($cells as $class => $content) {
				$class = is_numeric($class) ? '' : " class='so-$class'";
	...
	function table_row($cells, $id, $class) {
		$mk = $this->get_module_key();
		echo "\t<tr id='so-$mk-$id' class='so-$mk-$class'>\n";
		...
		foreach ($rows as $rowid => $row) {
			...
			foreach ($row as $class => $cell) {
				...
				if ($rowspans[$cellid] > 1)
					...
					echo "<td class='so-$mk-$class'$rowspanhtml>$cell</td>";
				}
	...
	function admin_form_group_start($title, $newtable=true) {
		$class = $newtable ? ' class="so-admin-form-group"' : '';
	...
	function admin_form_indent_start() {
		echo "<tr valign='top'><td colspan='2'><table class='so-indent'>";
	}
	...
	function textblock($text) {
		echo "<tr valign='top' class='so-admin-form-textblock'>\n<td colspan='2'>\n";
		echo $text;
		echo "\n</td>\n</tr>\n";
	}
	...
	function checkboxes($checkboxes, $grouptext=false, $args=array()) {
		...
		elseif ($output_tr)
			echo "<tr valign='top' class='so-admin-form-checkbox'>\n<td colspan='2'>\n";
		...
		if (is_array($checkboxes)) {
			foreach ($checkboxes as $name => $desc) {
				...
				if ($indent) $labelclass = " class='so-indent'"; else $labelclass = '';
	...
	function radiobuttons($name, $values, $grouptext=false) {
		...
		if ($grouptext)
			...
			echo "<tr valign='top' class='so-admin-form-radio'>\n<td colspan='2'>\n";
		...
		if (is_array($values)) {
			...
			foreach ($values as $value => $desc) {
				...
				if (!sostr::has($label, '</label>')) echo '</label>';
				//if (!sostr::has($desc,  '</label>')) echo '<br />';
				echo "</div>\n";
			}
		}
	...
	function dropdown($name, $values, $grouptext=false, $text='%s') {
		...
		if ($grouptext)
			...
			echo "<tr valign='top' class='so-admin-form-dropdown'>\n<td colspan='2'>\n";
		...
		if (is_array($values)) {
			...
			$dropdown =   "<select name='$name' id='$name'>\n"
						. sohtml::option_tags($values, $this->get_setting($name))
						. "</select>";
	...
	function textboxes($textboxes, $defaults=array(), $grouptext=false, $args=array()) {
		...
		if ($tree_level !== false) {
			$indentattrs = " class='so-indent so-indent-level-{$tree_level}'";
			if ($is_ec_tree) {
				if ($is_tree_parent)
					$indenttoggle = "<span class='so-child-fields-toggle'>+</span> ";
				else
					$indenttoggle = "<span class='so-child-fields-toggle-filler'> </span> ";
		...
		foreach ($textboxes as $id => $title) {
			...
			$value = so_esc_editable_html($this->get_setting($id));
			$id = so_esc_attr($id);
			$resetmessage = so_esc_attr(__('Are you sure you want to replace the textbox contents with this default value?', 'seo-optimization'));
			
			if ($grouptext)
				echo "<div class='field'><label for='$id'>$title</label><br />\n";
			elseif (strpos($title, '</a>') === false)
				echo "<tr valign='top'$indentattrs$hidden>\n<th scope='row' class='so-field-label'>$indenttoggle<label for='$id'><span class='so-field-label-text'>$title</span></label></th>\n<td>";
			else
				echo "<tr valign='top'$indentattrs$hidden>\n<td class='so-field-label'>$indenttoggle<span class='so-field-label-text'>$title</span></td>\n<td>";
			...
			elseif (isset($defaults[$id])) {
				$default = so_esc_editable_html($defaults[$id]);
				echo "onkeyup=\"javascript:so_textbox_value_changed(this, '$default', '{$id}_reset')\" />";
				echo "&nbsp;<a href=\"#\" id=\"{$id}_reset\" onclick=\"javascript:so_reset_textbox('$id', '$default', '$resetmessage', this); return false;\"";
				if ($default == $value) echo ' class="hidden"';
				echo ">";
				_e('Reset', 'seo-optimization');
				echo "</a>";
			...
			if (isset($args['open_url_value_link'])) {
				...
				echo so_esc_html($args['open_url_value_link']);
				echo '</a>';
			}
	...
	function get_nonce_handle($action, $id = false) {
		...
		if (strcmp($action, 'update') == 0) {
			...
			$handle = SO_PLUGIN_NAME."-$hook-$action$id";
	...
	function print_messages() {
		foreach ($this->messages as $type => $messages) {
			...
			if ($messages) {
				$type = so_esc_attr($type);
				echo "<div class='so-message'><p class='so-$type'>$messages</p></div>\n";
			}
		}
	...
	function print_mini_message($type, $message) {
		$type = so_esc_attr($type);
		echo "<div class='so-status so-$type'>$message</div>";
	}
	...
	function get_postmeta($key, $id=false) {
		...
		if (!$id) {
			//This code is different from sowp::get_post_id();
			if (is_admin()) {
		...
		if ($id) {
			...
			$value = get_post_meta($id, "_so_$key", true);
			$value = apply_filters("so_get_postmeta", $value, $key, $_post);
			$value = apply_filters("so_get_postmeta-$key", $value, $key, $_post);
		} else
			$value = '';
	...
	function get_postmeta_textboxes($textboxes) {
		...
		foreach ($textboxes as $id => $title) {
			...
			register_setting('seo-optimization', $id);
			$value = so_esc_editable_html($this->get_postmeta($id));
			$id = "_so_".so_esc_attr($id);
			//$title = str_replace(' ', '&nbsp;', $title);
			
			$html .= "<tr class='so textbox' valign='middle'>\n<th scope='row' class='so'><label for='$id'>$title</label></th>\n"
					."<td class='so'><input name='$id' id='$id' type='text' value='$value' class='regular-text' tabindex='2' /></td>\n</tr>\n";
		}
	...
	function get_postmeta_textareas($textareas) {
		...
		foreach ($textareas as $id => $title) {
			...
			$html .= "<tr class='so textarea' valign='top'>\n<th scope='row' class='so'><label for='$id'>$title</label></th>\n"
					."<td class='so'><textarea name='$id' id='$id' class='regular-text' tabindex='2' cols='60' rows='3'>$value</textarea></td>\n</tr>\n";
		}
	...
	function get_postmeta_checkboxes($checkboxes, $grouptext) {
		...
		$html = "<tr class='so checkboxes' valign='$valign'>\n<th scope='row' class='so'>$grouptext</th>\n<td class='so'><fieldset><legend class='hidden'>$grouptext</legend>\n";
		
		if (is_array($checkboxes)) {
			foreach ($checkboxes as $name => $desc) {
				
				register_setting('seo-optimization', $name);
				$checked = ($this->get_postmeta($name) == 1);
				$name = "_so_".so_esc_attr($name);
	...
	function get_postmeta_dropdown($name, $options, $grouptext) {
		...
		$html = "<tr class='so dropdown' valign='middle'>\n<th scope='row' class='so'><label for='$name'>$grouptext</label></th>\n<td class='so'><fieldset><legend class='hidden'>$grouptext</legend>\n";
		$html .= "<select name='$name' id='$name' onchange='javascript:so_toggle_select_children(this)'>\n";
		$html .= sohtml::option_tags($options, $current);
		$html .= "</select>\n";
		$html .= "</fieldset></td>\n</tr>\n";
	...
	function get_postmeta_subsection($field, $value, $html) {
		...
		$field = so_esc_attr($field);
		$value = so_esc_attr($value);
		$html = str_replace('<tr ', "<tr class='so_{$field}_{$value}_subsection$hidden' ", $html);
		return $html;
	}
	...
	function get_termmeta($key, $id=false, $module=false) {
		...
		$tax_meta = $this->get_setting(sostr::startwith($key, 'taxonomy_'), array(), $module);
	...
	function cron($function, $recurrence) {
		...
		$hook = "so-$mk-".str_replace('_', '-', $function);
		$start = time();
	 ...
	 * @uses SEO_Optimization::queue_js()
	 * @uses SEO_Optimization::queue_css()
	 */
	...
	/**
	 * Outputs the SEO Optimization XMLNS used by JLSuggest.
	 * 
	 * @since 6.0
	 */
	function jlsuggest_xml_ns() {
		echo ' xmlns:so="http://johnlamansky.com/xmlns/seo-optimization" ';
	}
	...
	function jlsuggest_value_explode($valstr) {
		...
		if (is_array($valstr)) {
			...
			if (sostr::startswith($valstr, 'obj_')) {
				$valstr = sostr::ltrim_str($valstr, 'obj_');
	...
	function get_jlsuggest_box($name, $value, $params='') {
		...
		switch ($to_genus) {
			...
			case 'posttype':
				...
				if ($selected_post) {
					...
					if ($selected_post_type)
						$text_dest = sprintf(__('A Deleted %s', 'seo-optimization'), $selected_post_type->labels->singular_name);
					else
						$text_dest = __('A Deleted Post', 'seo-optimization');
			...
			case 'taxonomy':
				if ($selected_taxonomy = get_taxonomy($to_type)) {
					if ($selected_term = get_term($to_id, $selected_taxonomy->name)) {
						...
						$text_dest = sprintf(__('A Deleted %s', 'seo-optimization'), $selected_taxonomy->labels->singular_name);
						$text_dest = '<span class="type">' . $text_dest . '</span>';
						$disabled = true;
					}
				} else {
					$text_dest = __('A Deleted Term', 'seo-optimization');
					$text_dest = '<span class="type">' . $text_dest . '</span>';
					$disabled = true;
				}
				break;
			case 'home':
				$text_dest = __('Blog Homepage', 'seo-optimization');
				break;
			case 'author':
				if (is_user_member_of_blog($to_id)) {
					$selected_author = get_userdata($to_id);
					$text_dest = $selected_author->user_login . '<span class="type">&nbsp;&mdash;&nbsp;'.__('Author', 'seo-optimization').'</span>';
				} else {
					$text_dest = __('A Deleted User', 'seo-optimization');
					$text_dest = '<span class="type">' . $text_dest . '</span>';
					$disabled = true;
				}
				break;
			case 'internal-link-alias':
				...
				if (isset($aliases[$to_id]['to'])) {
					$h_alias_to = so_esc_html($aliases[$to_id]['to']);
					$text_dest = "/$alias_dir/$h_alias_to/" . '<span class="type">&nbsp;&mdash;&nbsp;';
					
					if ($this->plugin->module_exists('internal-link-aliases')) {
						$text_dest .= __('Link Mask', 'seo-optimization');
					} else {
						$text_dest .= __('Link Mask (Disabled)', 'seo-optimization');
						$disabled = true;
					}
					$text_dest .= '</span>';
				} else {
					$text_dest = __('A Deleted Link Mask', 'seo-optimization');
					$text_dest = '<span class="type">' . $text_dest . '</span>';
					$disabled = true;
				}
		...
		$html .= so_esc_editable_html($is_url ? $to_id : $obj);
		$html .= "'";
		
		if ($params) {
			$e_params = so_esc_attr($params);
			$html .= " so:params='$e_params'";
		}
		
		$html .= " type='text' class='textbox regular-text jlsuggest'";
		$html .= ' title="' . __('Type a URL or start typing the name of an item on your site', 'seo-optimization') . '"';
		...
		$html .= $text_dest;
		$html .= '</div>';
		$html .= '<div><a href="#" onclick="javascript:return false;" class="jls_text_dest_close" title="'.__('Remove this location from this textbox', 'seo-optimization').'">'.__('X', 'seo-optimization').'</a></div>';
		$html .= '</div>';
	...
	function jlsuggest_value_to_url($value) {
		...
		switch ($to_genus) {
			...
			case 'taxonomy':
				...
				break;
			case 'home':
				return sowp::get_blog_home_url(); break;
			case 'author':
				$to_id = (int)$to_id;
				if (is_user_member_of_blog($to_id))
