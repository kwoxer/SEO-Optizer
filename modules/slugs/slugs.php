...
class SO_Slugs extends SO_Module {
	
	function get_module_title() { return __('Slug Optimizer', 'seo-optimization'); }
	
	function admin_page_contents() {
		$this->admin_form_start();
		$this->textarea('words_to_remove', __('Words to Remove', 'seo-optimization'), 20);
		$this->admin_form_end();
	}
	...
	function optimize_slug($slug) {
		...
		$newslug = implode("-", array_diff(explode(" ", $slug), soarr::explode_lines($words)));
...
", 'seo-optimization')));
	...
	$screen->add_help_tab(array(
			  'id' => 'so-rich-snippets-troubleshooting'
