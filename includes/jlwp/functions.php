...
define('SOWP_QUERY_PERMALINKS', 0);
define('SOWP_INDEX_PERMALINKS', 1);
define('SOWP_PRETTY_PERMALINKS', 2);
...
class sOwp {
	...
	function get_any_posts($args = array()) {
		...
		$args['post_type'] = sowp::get_post_type_names(); //...as opposed to "any" because then get_posts() will remove search-excluded post types
		$args['numberposts'] = -1;
	...
	function get_post_type_objects() {
		...
		/*
		if (function_exists('get_post_types'))
			$types = soarr::flatten_values(get_post_types(array('public' => true), 'objects'), array('labels', 'name'));
		*/
	...
	function get_taxonomies() {
		...
		if (isset($taxonomies['post_format']) && $taxonomies['post_format']->labels->name == _x( 'Format', 'post format' ))
			$taxonomies['post_format']->labels->name = __('Post Format Archives', 'seo-optimization');
		return $taxonomies;
	}
	...
	function add_backup_url($text) {
		$anchor = __('backup your database', 'seo-optimization');
		return str_replace($anchor, '<a href="'.sowp::get_backup_url().'" target="_blank">'.$anchor.'</a>', $text);
	}
	...
	function remove_instance_action($tag, $class, $function, $priority=10) {
		return sowp::remove_instance_filter($tag, $class, $function, $priority);
	}
	...
	function remove_instance_filter($tag, $class, $function, $priority=10) {
		if (isset($GLOBALS['wp_filter'][$tag][$priority]) && count($GLOBALS['wp_filter'][$tag][$priority])) {
			foreach ($GLOBALS['wp_filter'][$tag][$priority] as $key => $x) {
				if (sostr::startswith($key, $class.$function)) {
	...
	function permalink_mode() {
		if (strlen($struct = get_option('permalink_structure'))) {
			if (sostr::startswith($struct, '/index.php/'))
				return SOWP_INDEX_PERMALINKS;
			else
				return SOWP_PRETTY_PERMALINKS;
		} else
			return SOWP_QUERY_PERMALINKS;
	}
