...
class soio {
	...
	function is_file($filename, $path, $ext=false) {
		$is_ext = strlen($ext) ? sostr::endswith($filename, '.'.ltrim($ext, '*.')) : true;		
		return is_file(soio::tslash($path).$filename) && $is_ext;
	}
	
	function is_dir($name, $path) {
		return $name != '.' && $name != '..' && is_dir(soio::tslash($path).$name);
	}
	
	function tslash($path) {
		return soio::untslash($path).'/';
	}
	...
	function print_csv($csv) {
		...
		foreach ($csv as $row) {
			...
			foreach ($headers as $header) {
				...
				if (sostr::has($csv_row[$header], ',')) $csv_row[$header] = '"'.$csv_row[$header].'"';
			}
