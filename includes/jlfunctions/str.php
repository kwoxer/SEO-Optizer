...
class sostr {
	...
	function rtrim_str($str, $totrim) {
		if (strlen($str) > strlen($totrim) && sostr::endswith($str, $totrim))
	...
	function rtrim_substr($str, $totrim) {
		for ($i = strlen($totrim); $i > 0; $i--) {
			...
			if (sostr::endswith($str, $totrimsub))
				return sostr::rtrim_str($str, $totrimsub);
		}
	...
	function ltrim_str($str, $totrim) {
		if (strlen($str) > strlen($totrim) && sostr::startswith($str, $totrim))
	...
	function to_int($str) {
		return intval(sostr::preg_filter('0-9', strval($str)));
	}
	...
	function htmlsafe_str_replace($search, $replace, $subject, $limit, &$count, $exclude_tags = false) {
		$search = sostr::preg_escape($search);
		return sostr::htmlsafe_preg_replace($search, $replace, $subject, $limit, $count, $exclude_tags);
	}
	...
	function htmlsafe_preg_replace($search, $replace, $subject, $limit, &$count, $exclude_tags = false) {
		...
		if (count($exclude_tags) > 1)
			$exclude_tags = sostr::preg_filter('a-z0-9|', implode('|', $exclude_tags));
		else
