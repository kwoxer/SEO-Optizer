...
class soarr {
	...
	function search_recursive($needle, $haystack) {
		foreach ($haystack as $key => $value) {
			if ($needle === $value || (is_array($value) && soarr::search_recursive($needle, $value) !== false))
				return $key;
		}
		return false;
	}
	...
	function recursive_get($array, $key) {
		if (is_array($array)) {
			...
			foreach ($array as $subarray) {
				if ($value = soarr::recursive_get($subarray, $key))
					return $value;
			}
		}
	...
	function vksort(&$arr, $valuekey) {
		$valuekey = sostr::preg_filter('A-Za-z0-9 ', $valuekey);
	...
	function flatten_values($arr, $value_keys) {
		foreach ((array)$value_keys as $key)
			$arr = soarr::_flatten_values($arr, $key);
		return $arr;
	}
	...
	function key_replace($array, $key_changes, $recursive = true, $return_replaced_only = false) {
		...
		foreach ($array as $key => $value) {
			...
			if ($recursive && is_array($value)) {
				$oldvalue = $value;
				$value = soarr::key_replace($value, $key_changes, true, $return_replaced_only);
	...
	function value_replace($array, $value_changes, $recursive = true, $return_replaced_only = false) {
		...
		foreach ((array)$array as $key => $value) {
			...
			if ($recursive && is_array($value))
				$value = soarr::value_replace($value, $value_changes, true);
	...
	function simplify($arr, $keyloc, $valloc) {
		$keys = soarr::flatten_values($arr, $keyloc);
		$values = soarr::flatten_values($arr, $valloc);
