...
class SEO_Optimization {
	 ...
	 * @uses plugin_update_info() Hooked into the "in_plugin_update_message-seo-optimization/seo-optimization.php" action.
	...
	function __construct($plugin_file) {
		...
		$psdata = get_option('seo_optimization', array());
		...
		if (isset($psdata['version']))
			...
			$oldversion = get_option('so_version', false);
		...
		if ($oldversion != $version) update_option('seo_optimization', $psdata);
		...
		if (is_admin()) {
			...
			if (!get_option('blog_public')) {
				...
				sowp::remove_instance_action('admin_footer', 'RobotsMeta_Admin', 'blog_public_warning');
			}
			...
			//JLSuggest AJAX
			add_action('wp_ajax_so-jlsuggest-autocomplete', array(&$this, 'jlsuggest_autocomplete'));
		}
	}
	 ...
	 * @uses __construct()
	 */
	function SEO_Optimizazion($plugin_file) {
	...
	function upgrade($oldversion) {
		define('SO_UPGRADE', true);
	}
	
	/**
	 * Upgrades SEO Optimizazion to version 0.8.
	 * 
	 * @since 0.8
	 */
	function upgrade_to_08() {
		...
		foreach ($options as $option) {
			if (($value = get_option("so_$option", false)) !== false) {
				
				if ('settings' == $option) {
					foreach ((array)$value as $module => $module_data)
						update_option("seo_optimization_module_$module", $module_data);
				} else {
					$psdata[$option] = $value;
					$save = true;
				}
				
				delete_option("so_$option");
			}
		}
		if ($save) update_option('seo_optimization', $psdata);
	}
	
	/**
	 * Upgrades SEO Optimizazion to version 4.0.
	 * 
	 * @since 4.0
	 */
	...
	/**
	 * Upgrades SEO Optimization to version 5.0.
	 * 
	 * @since 5.0
	 */
	...
	function copy_module_states($copy) {
		...
		if ($save) update_option('seo_optimization', array());
	}
	...
	function uninstall() {
		...
		if (!empty($psdata['modules'])) {
			...
			foreach ($module_keys as $module)
				delete_option("seo_optimization_module_$module");
		}
		
		//Delete plugin data
		delete_option('seo_optimization');
	}
	 ...
	 * 
	 * SEO Optimization uses a modular system that allows functionality to be added and removed on-the-fly.
	 * 
	 * @since 0.1
	 * @uses $plugin_dir_path
	 ...
	 * @uses SO_MODULE_ENABLED
	 * @uses SO_MODULE_DISABLED
	...
	function load_modules() {
					...
					if (soio::is_file($file, $subdirpath, 'php') && $file != 'index.php') {
						...
						if (class_exists($class)) {
							...
							if (	   ($module_parent = call_user_func(array($class, 'get_parent_module')))
								...
								$module_disabled = (isset($oldmodules[$module_parent]) && $oldmodules[$module_parent] == SO_MODULE_DISABLED);
							else
								$module_disabled = (isset($oldmodules[$module]) && $oldmodules[$module] == SO_MODULE_DISABLED);
		...
		foreach ($this->disabled_modules as $key => $name) {
			$newmodules[$key] = SO_MODULE_DISABLED;
		}
		
		//Save the new modules list
		$psdata['modules'] = $newmodules;
		if ($newmodules != $oldmodules) update_option('seo_optimization', $psdata);
	 ...
	 * @since 0.1
	 * @uses $plugin_file_path
	 * @uses SO_Module::load_default_settings()
	 * @uses SO_Module::init()
	 */
	function init() {
	...
	function load_textdomain() {
		load_plugin_textdomain('seo-optimization', '', trailingslashit(plugin_basename($this->plugin_dir_path)) . 'translations');
	}
	 ...
	 * @since 6.0
	 * @uses $modules
	 * @uses SO_Module::is_module_admin_page()
	 * @uses SO_Module::admin_page_init()
	 */
	function admin_init() {
	...
	function save_hit() {
		...
		if (!empty($this->hit) && $this->get_setting('log_hits', true, 'settings'))
			do_action('so_save_hit', $this->hit);
	}
	 ...
	 * @uses SO_Module::get_menu_count()
	 * @uses SO_Module::get_menu_pos()
	 * @uses SO_Module::get_menu_title()
	 * @uses SO_Module::get_page_title()
	 * @uses key_to_hook()
	 */
	function add_menus() {
		...
		$count = 0;
		foreach ($this->modules as $key => $module) {
			if (	(empty($psdata['modules']) || $psdata['modules'][$key] > SO_MODULE_SILENCED)
		...
		add_utility_page(__('SEO Optimization', 'seo-optimization'), __('SEO', 'seo-optimization').$count_code, 'manage_options', 'seo', array(), 'div');
		...
		foreach ($this->modules as $key => $x_module) {
			...
			if ($module->get_menu_title() && (!$module->get_parent_module() || !$this->module_exists($module->get_parent_module()))) {
				...
				if (empty($psdata['modules']) || $psdata['modules'][$key] > SO_MODULE_HIDDEN)
					$parent = $module->get_menu_parent();
				else
					$parent = 'so-hidden-modules';
	...
	function key_to_hook($key) {
		switch ($key) {
			...
			case 'settings': return 'seo-optimization'; break;
			default: return "so-$key"; break;
		}
	}
	...
	function hook_to_key($hook) {
		switch ($hook) {
			...
			case 'seo-optimization': return 'settings'; break;
		...
		if (strlen($pp)) {
			...
			foreach ($this->modules as $key => $module) {
				...
				if (strcmp($key, $pp) == 0)
					...
					echo "\n<script type='text/javascript'>pagenow = '".so_esc_attr($module->plugin_page_hook)."';</script>\n";
				...
				if (strcmp($key, $pp) == 0 || strcmp($module->get_parent_module(), $pp) == 0) {
					...
					if (!$outputted_module_files) {
						...
						$this->queue_js ('modules', 'modules', array('jquery'), array(
							'unloadConfirmMessage' => __("It looks like you made changes to the settings of this SEO Optimization module. If you leave before saving, those changes will be lost.", 'seo-optimization')
						));
	...
	function queue_file($reldir, $filename, $ext, $func, $deps=array(), $l10n=array()) {
		...
		$relurl = $reldir . '/';
		$file = sostr::endwith($filename, $ext);
		if (file_exists($this->plugin_dir_path.$relurl.$file))
			$func("so-$dirid-$filename", $this->plugin_dir_url.$relurl.$file, $deps, SO_VERSION);
		
		if (count($l10n))
			wp_localize_script("so-$dirid-$filename", sostr::camel_case("so $dirid $filename l10n"), $l10n);
	}
	...
	function admin_help($text, $screen) {
		...
		if (strcmp($screen, 'post') == 0 || strcmp($screen, 'page') == 0) {
			...
			$helparray = apply_filters('so_postmeta_help', array());
			...
			if ($helparray) {
				...
				$text .= "<div class='so-help'>\n";
				$text .= '<h5>'.__('SEO Settings Help', 'seo-optimization')."</h5>\n";
				$text .= "<div class='metabox-prefs'>\n";
				$text .= "<p>".__('The SEO Settings box lets you customize these settings:', 'seo-optimization')."</p>\n";
				$text .= "<ul>\n$customhelp\n</ul>";
				$text .= "<p><em>".__('(The SEO Settings box is part of the SEO Optimization plugin.)', 'seo-optimization')."</em></p>\n";
				$text .= "\n</div>\n</div>\n";
				return $text;
			}
		}
	...
	/**
	 * Notifies the user if he/she is using plugins whose functionality SEO Optimization replaces.
	 * 
	 * @since 0.1
	...
	/**
	 * Outputs a table row notifying the user that he/she is using a plugin whose functionality SEO Optimization replaces.
	 * 
	 * @since 0.1
	 */
	function plugin_page_notice($file, $data, $context) {
		if (is_plugin_active($file)) {
			...
			echo "<tr class='plugin-update-tr so-plugin-notice'><td colspan='$columns' class='plugin-update'><div class='update-message'>\n";
			printf(__('SEO Optimization includes the functionality of %1$s. You may want to deactivate %1$s to avoid plugin conflicts.', 'seo-optimization'), $data['Name']);
			echo "</div></td></tr>\n";
		}
	}
	...
	function plugin_update_info($plugin_data, $r) {
		...
		if ($r && $r->new_version && !is_plugin_active('changelogger/changelogger.php'))
			...
			if ($info = $this->get_plugin_update_info($r->new_version))
				...
				echo "<span class='so-plugin-update-info'><br />$info</span>";
	}
	
	/**
	 * Filters the list of plugin action links for SEO Optimization and adds links to certain modules if those modules are enabled.
	 * 
	 * @since 2.1
	 * 
	...
	function plugin_action_links($actions) {
		$so_actions = array(
			  'uninstall' => __('Uninstall', 'seo-optimization')
		);
		
		foreach ($so_actions as $module => $anchor) {
	...
	/**
	 * Outputs a list of active modules on SEO Optimization's plugin page listing.
	 * 
	 * @since 2.1
	 */
	function plugin_row_meta_filter($plugin_meta, $plugin_file) {
		if ($plugin_file == $this->plugin_basename)
			echo $this->get_module_links_list('<p id="so-active-modules-list">'.__('Active Modules: ', 'seo-optimization'), ' | ', '</p>');
	...
	function get_module_links_list($before = '', $between = ' | ', $after = '') {
		...
		if (count($this->modules)) {
			...
			foreach ($this->modules as $key => $x_module) {
				$module =& $this->modules[$key];
				if (strcasecmp(get_parent_class($module), 'SO_Module') == 0 && $module->is_independent_module()) {
			...
			foreach ($modules as $title => $url) {
				$url = so_esc_attr($url);
				$title = str_replace(' ', '&nbsp;', so_esc_html($title));
	...
	function private_blog_admin_notice() {
		echo "\n<div class='error'><p>";
		_e('<strong>SEO Optimization Notice:</strong> Your blog is configured to block search engine spiders. To resolve this, <a href="options-privacy.php" target="_blank">go to your Privacy settings</a> and set your blog visible to everyone.', 'seo-optimization');
		echo "</p></div>";
	}
	 ...
	 * 
	 * @since 0.8
	 * @uses SO_Module::postmeta_fields()
	 * 
	 * @param string $screen The admin screen currently being viewed (post, page). Defaults to post. Optional.
	...
	function add_postmeta_box() {
		...
		$posttypes = sowp::get_post_type_names();
		foreach ($posttypes as $screen) {
			
			//Only show the meta box if there are fields to show.
			if ($this->get_postmeta_fields($screen))
				add_meta_box('so_postmeta', __('SEO Settings', 'seo-optimization'), create_function('', 'global $seo_optimization; $seo_optimization->show_postmeta_box("'.$screen.'");'), $screen, 'normal', 'high');
		}
	}
	...
	function show_postmeta_box($screen) {
		...
		//Begin box
		echo "<div id='so-postmeta-box'>\n";
		wp_nonce_field('so-update-postmeta', '_so_wpnonce');
		echo "\n<table>\n";
	...
	function save_postmeta_box($post_id, $post) {
		...
		if ( !isset($_REQUEST['_so_wpnonce']) || !wp_verify_nonce($_REQUEST['_so_wpnonce'], 'so-update-postmeta') ) return;
		...
		foreach ($fields as $field) {
			...
			if (!apply_filters("so_custom_update_postmeta-$field", false, $value, $metakey, $post)) {
				if (empty($value))
	...
	function template_head() {
		...
		if (get_option('blog_public')) {
			$robots = implode(',', apply_filters('so_meta_robots', array()));
			if ($robots) echo "\t<meta name=\"robots\" content=\"$robots\" />\n";
		}
		
		if ($markcode) echo "<!-- /".SO_PLUGIN_NAME." -->\n\n";
	}
	
	/**
	 * Marks code with HTML comments identifying SEO Optimization, if the user has set this option.
	 * 
	 * @since 2.7
	 */
	function mark_code($code, $info = '', $info_only = false) {
		...
		if ($this->get_setting('mark_code', false, 'settings')) {
			...
			if ($info_only)
				...
				if ($info) $info = " - $info";
				$start = sprintf('%s (%s)%s', SO_PLUGIN_NAME, SO_PLUGIN_URI, $info);
				$end = SO_PLUGIN_NAME;
			}
	...
	function jlsuggest_autocomplete() {
		...
		if ((!$include || in_array('home', $include)) && sostr::ihas($_GET['q'], 'home')) {
			$items[] = array('text' => __('Home', 'seo-optimization'), 'isheader' => true);
			$items[] = array('text' => __('Blog Homepage', 'seo-optimization'), 'value' => 'obj_home', 'selectedtext' => __('Blog Homepage', 'seo-optimization'));
		}
		
		if (!$include || in_array('posttype', $include)) {
			$posttypeobjs = sowp::get_post_type_objects();
			foreach ($posttypeobjs as $posttypeobj) {
				...
				soarr::remove_value($stati, 'auto-draft');
				$stati = implode(',', $stati);
		...
		if (!$include || in_array('taxonomy', $include)) {
			$taxonomyobjs = sowp::get_taxonomies();
		...
		if (!$include || in_array('author', $include)) {
			...
			if (count($authors)) {
				...
				$items[] = array('text' => __('Author Archives', 'seo-optimization'), 'isheader' => true);
				
				foreach ($authors as $author)
					$items[] = array(
						  'text' => $author->user_login
						, 'value' => 'obj_author/' . $author->ID
						, 'selectedtext' => $author->user_login . '<span class="type"> &mdash; '.__('Author', 'seo-optimization').'</span>'
					);
			}
		}
					...
					if ($alias['to']) {
						...
						if ((strpos($alias['from'], $_GET['q']) !== false) || (strpos($to_rel_url, $_GET['q']) !== false)) {
							...
							$items[] = array(
								...
								, 'selectedtext' => $to_rel_url . '<span class="type"> &mdash; '.__('Link Mask', 'seo-optimization').'</span>'
							);
